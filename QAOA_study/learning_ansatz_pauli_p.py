...
if __name__ == "__main__":
    ...
    for p in p_list: 
        ...
        for i in range(len(instances)): 
            ...
            keys=list(noisy_counts[3].keys())
            
            # List comprehension to replace '0' with 'I' and '1' with 'Z' for each string in keys
            pauli_operators = [''.join(['I' if char == '0' else 'Z' for char in key]) for key in keys] #list of the pauli strings  

            optimal_a_list=[]
            optimal_b_list=[] #list with the opitmal parameters to correct the noisy value of a pauli operator 
            for pauli_string in pauli_operators: 
                # for each pauli 
                noisy_values=[]
                cliff_values=[]

                for noisy_dict,cliff_dict in zip(noisy_counts,cliff_counts): 
                    noisy_values.append(calculate_expect_value_from_counts(noisy_dict,pauli_string,shots=shots))
                    cliff_values.append(calculate_expect_value_from_counts(cliff_dict,pauli_string,shots=shots))
                
                #we have x,y data
                # Use curve_fit to find optimal values of a 
                params, covariance = curve_fit(linear_ansatz, noisy_values, cliff_values , p0=[1.0,0.0])
                # Extract the optimized values of a
                a_opt, b_opt = params
            ...
            write_in_file(file_name,[pauli_operators,optimal_a_list,optimal_b_list],header='#bitstring  ,optimal_params_pauli ')
