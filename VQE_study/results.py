...
if __name__ == "__main__":
    ...
    relative_error_pauli=[]

    relative_error_noisy_pm=[]
    relative_error_direct_pm=[]
    relative_error_pauli_pm=[]
    ...
    for g in g_list: 
        ...
        mitigated_hamiltonian_direct_list_j=[]
        mitigatated_hamiltonian_pauli_list_j=[]
        ...
        for l in range(10): 
            ...
            pauli_strings, noisy_results=expectation_values_to_mitigate(noisy_o_countsZ,noisy_o_countsX,n_qubits,hamiltonian)
            pauli_strings, ideal_results=expectation_values_to_mitigate(ideal_o_countsZ,ideal_o_countsX,n_qubits,hamiltonian)
            ...
            i=-1
            for pauli_string,a,b in zip(pauli_strings,a_optimal_list,b_optimal_list): 
                i=i+1
                if i!=len(pauli_strings)-1: 
                    mitigated_results.append(linear_ansatz(noisy_results[i],a,b))
                else: 
                    mitigated_results.append(linear_ansatz(noisy_results[i],a,b))

            noisy_hamiltonian_list_j.append(noisy_results[-1])
            ideal_hamiltonian_list_j.append(ideal_results[-1])
            mitigated_hamiltonian_direct_list_j.append(mitigated_results[-1])
            mitigatated_hamiltonian_pauli_list_j.append(compute_hamiltonian_from_paulis(mitigated_results,n_qubits,hamiltonian)) 
        ...
        relative_error_direct_j=[np.abs(i_h-n_h)*100/np.abs(i_h) for i_h,n_h in zip(ideal_hamiltonian_list_j,mitigated_hamiltonian_direct_list_j)]
        relative_error_pauli_j=[np.abs(i_h-n_h)*100/np.abs(i_h) for i_h,n_h in zip(ideal_hamiltonian_list_j,mitigatated_hamiltonian_pauli_list_j)]
        ...
        relative_error_direct.append(direct)
        relative_error_direct_pm.append(direct_pm)

        relative_error_pauli.append(pauli)
        relative_error_pauli_pm.append(pauli_pm)


    file_name='results/results_relative_error_g_average_instances_VQE_'+str(shots)+'_qubits'+str(n_qubits)
    write_in_file(file_name,[g_list,relative_error_noisy,relative_error_noisy_pm,relative_error_direct,relative_error_direct_pm,relative_error_pauli,relative_error_pauli_pm],header='#g, ideal, noisy, mit, mit pauli ')
